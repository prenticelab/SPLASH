...
# description:
# ~~~~~~~~~~~~
# This script runs SPLASH for point-based data.
#
# ~~~~~~~~~~
# changelog:
# ~~~~~~~~~~
...
# * Name: spin_up
# *
# * Input: list, meteorological data (mdat)
...
spin_up <- function(mdat, dtot){
  ...
  for (i in seq(from=1, to=mdat$num_lines, by=1)){
    ...
    my_vals <- quick_run(mdat$lat_deg, mdat$elv_m, i, mdat$year, wn, mdat$sf[i],
                         mdat$tair[i], mdat$pn[i])
